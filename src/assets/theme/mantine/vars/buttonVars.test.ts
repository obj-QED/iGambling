...
describe('resolveButtonRootVars', () => {
  ...
  it('applies component + key CMF cascade when scope attrs are set', () => {
    const vars = resolveButtonRootVars(theme, {
      variant: 'filled',
      'data-cmf-component': 'header',
      'data-menu-key': 'sign_in',
    });

    expect(vars['--button-bg']).toContain('--cmf-header-sign_in-button-filled-bg');
    expect(vars['--button-bg']).toContain('--cmf-header-button-filled-bg');
    expect(vars['--button-bg']).toContain('--cmf-button-filled-bg');
  });
});
